...
@author: acer
def returnpoint(points=[0,0], boundingbox=[0,0,0,0])
    pointsimage=[0,0]
    pointsimage[0]=points[0]+boundingbox[0]
    pointsimage[1]=points[1]+boundingbox[1]
    return pointsimage
"""
...
retval2, im_at_fixed2 = cv2.threshold(img, 60, 255, cv2.THRESH_BINARY_INV)
kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(3, 3))
eroded = cv2.erode(im_at_fixed,kernel)

#img1 = Image.fromarray(im_at_fixed2)
#img1.show()
...
#
#内部孔填充算法，从（0,0）开始
im_floodfill = im_at_fixed2.copy()
# Mask used to flood filling.
# Notice the size needs to be 2 pixels than the image.
h, w = im_at_fixed2.shape[:2]
...
im_out = im_at_fixed2 | im_floodfill_inv
...
for i in range(0,mosquitoesnum):
    ...
    if output1[0]!=1:
        ...
        cropimage0 = im_out[rect0a[0]:rect0a[2],rect0a[1]:rect0a[3]]
        cropimage1 = im_out[rect1a[0]:rect1a[2],rect1a[1]:rect1a[3]]
...
#cropimage1 = im_out[rect1a[0]:rect1a[2],rect1a[1]:rect1a[3]]   
#img1 = Image.fromarray(cropimage1)
#img1.show()